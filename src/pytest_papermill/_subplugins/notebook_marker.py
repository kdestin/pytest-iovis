...
class NotebookMarkerHandler:
    ...
    @classmethod
    def get_notebook_path(cls, item: pytest.Item) -> Optional[Path]:
    ...
    def _remove_overwritten_default_tests(self, items: List[pytest.Item]) -> List[pytest.Item]:
        """Remove tests generated by JupyterNotebookFile if there exists a user-defined test for that same notebook.

        :param List[pytest.Item] items: The list of tests
        :return: The filtered list of tests
        :rtype: List[pytest.Item]
        """

        def is_user_defined(item: pytest.Item) -> bool:
            """Whether a marked pytest.Function was defined by the user"""
            return self.is_marked_function(item) and not JupyterNotebookFile.is_plugin_generated(item)

        # Remove plugin generated test functions that should be shadowed by a user-defined one.
        user_defined_paths = {self.get_notebook_path(i) for i in items if is_user_defined(i)}

        return [
            i
            for i in items
            if not (JupyterNotebookFile.is_plugin_generated(i) and self.get_notebook_path(i) in user_defined_paths)
        ]
    ...
    def pytest_collection_modifyitems(self, items: List[pytest.Item]) -> None:
        ...
        items[:] = self._remove_overwritten_default_tests(items)
