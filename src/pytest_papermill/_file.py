...
class JupyterNotebookFile(pytest.Module):
    ...
    __GENERATED_BY_CLASS_KEY = pytest.StashKey[bool]()
    """A stash key that marks whether a pytest.Item was generated by this class."""
    ...
    def collect(self) -> Iterable[pytest.Function]:
        ...
        def mark_default(item: pytest.Item) -> pytest.Item:
            item.stash[self.__GENERATED_BY_CLASS_KEY] = True
            return item

        for f in self._test_functions:
            funcs = self.ihook.pytest_pycollect_makeitem(collector=self, name=f.__name__, obj=f) or []
            yield from map(mark_default, funcs)
    ...
    @classmethod
    def is_plugin_generated(cls, item: pytest.Item) -> bool:
        """Return whether a pytest.Item was collected by a JupyterNotebookFile"""
        return item.stash.get(cls.__GENERATED_BY_CLASS_KEY, False)
