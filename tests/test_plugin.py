...
def test_subplugin_names(testdir: pytest.Testdir) -> None:
    ...
    testdir.makepyfile(
        """
        from pytest_iovis._subplugins import IPythonMarkupPlugin, JupyterNotebookDiscoverer, NotebookMarkerHandler
        ...
        def test_plugin_names(pytestconfig):
            ...
            assert isinstance(pluginmanager.get_plugin("iovis.notebook_marker"), NotebookMarkerHandler)
        """
    )
